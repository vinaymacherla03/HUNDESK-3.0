...
const JobMatchAnalyzer: React.FC<JobMatchAnalyzerProps> = ({ resumeData, jobDescription, onJobDescriptionChange }) => {
    ...
    const { onRewrite } = useResumeContext();
        ...
        <div className="space-y-4">
                ...
                {isExpanded ? (
                    ...
                    >
                        ...
                        <textarea
                            ...
                            className="block w-full text-xs p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:bg-white transition-all resize-none mb-3"
            ...
            <div className="flex gap-2">
                ...
                <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold rounded-xl text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-lg shadow-blue-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
                    {isTailoring ? (
                        <>
                            <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
            ...
            <AnimatePresence>
                ...
                {tailoredContent && !isTailoring && (
                    <motion.div
                        ...
                        className="bg-indigo-50 rounded-2xl border border-indigo-100 shadow-sm p-4 space-y-4"
                    >
                        <div className="flex items-center gap-2">
                            <SparkleIcon className="w-4 h-4 text-indigo-600" />
                            <h4 className="text-sm font-bold text-indigo-900">Tailored Suggestions</h4>
                        </div>
                        ...
                        <div className="bg-white p-3 rounded-xl border border-indigo-100">
                            ...
                            <button 
                                ...
                                className="mt-3 w-full py-1.5 text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors"
                            >
                                Copy New Summary
                            </button>
                        </div>
                        ...
                        <div className="bg-white p-3 rounded-xl border border-indigo-100">
                            ...
                            <div className="flex flex-wrap gap-1.5">
                                {tailoredContent.suggestedSkills.map((skill, i) => (
                                    <span key={i} className="text-[10px] bg-indigo-50 text-indigo-700 px-2 py-1 rounded-md border border-indigo-100 font-medium">{skill}</span>
                                ))}
                            </div>
                        </div>
                    </motion.div>
