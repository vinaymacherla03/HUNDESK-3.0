...
const DirectorTemplate: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
  ...
  const sections: Record<ResumeSectionKey, React.ReactNode> = {
    summary: (
      <section key="summary" className="mb-6">
        <EditableField as="p" path="summary" value={data.summary} onChange={onDataChange} className="text-black text-center italic" />
      </section>
    ),
    experience: (
      <section key="experience" className="mb-6">
        <h3 style={sectionTitleStyles} className="font-semibold mb-4">Professional Experience</h3>
            ...
            <div key={exp.id || index} className="mb-5 break-inside-avoid">
              <div className="flex justify-between items-baseline">
                <h4 style={{ fontSize: `${customization.itemTitleSize}pt` }} className="font-semibold text-black">
                  <EditableField as="span" path={`experience[${index}].role`} value={exp.role} onChange={onDataChange} />, <EditableField as="span" path={`experience[${index}].company`} value={exp.company} onChange={onDataChange} className="font-normal italic" />
                </h4>
    ...
    education: (
        <section key="education" className="mb-6">
          <h3 style={sectionTitleStyles} className="font-semibold mb-4">Education</h3>
    ...
    skills: (
        <section key="skills" className="mb-6">
          <h3 style={sectionTitleStyles} className="font-semibold mb-4">Core Competencies</h3>
           <EditableList items={data.skills || []} path="skills" onChange={onDataChange} newItem={newSkillCategory}>
            {(category, catIndex) => (
                <p key={category.id || catIndex} className="text-black text-center">
    ...
    projects: (
        <section key="projects" className="mb-6">
          <h3 style={sectionTitleStyles} className="font-semibold mb-4">Projects</h3>
    ...
    certifications: (
        <section key="certifications" className="mb-6">
            <h3 style={sectionTitleStyles} className="font-semibold mb-4">Certifications</h3>
    ...
    awards: (
        <section key="awards" className="mb-6">
            <h3 style={sectionTitleStyles} className="font-semibold mb-4">Awards</h3>
