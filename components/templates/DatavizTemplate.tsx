...
import { ResumeData, ResumeSectionKey, Customization, Experience, Education, SkillCategory } from '../../types';
...
const DatavizTemplate: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
    ...
    experience: (
      <Section key="experience" title="Experience">
        <EditableList items={data.experience || []} path="experience" onChange={onDataChange} newItem={newExperienceEntry}>
          {(exp, index) => (
            <div key={exp.id || index} className="mb-5 break-inside-avoid relative pl-8">
                <div className="absolute left-0 top-1 w-5 h-5 rounded-full bg-[var(--primary-color)]" />
    ...
    <div id="resume-content" style={{ padding: marginValue }}>
        <header className="mb-8">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3">
                <div className={contactItemClass}><ContactIcon type="email" className="h-4 w-4 shrink-0 text-[var(--primary-color)]"/> <EditableField as="a" href={`mailto:${data.contactInfo.email}`} path="contactInfo.email" value={data.contactInfo.email} onChange={onDataChange} className={contactLinkClass} validation="email"/></div>
                <div className={contactItemClass}><ContactIcon type="phone" className="h-4 w-4 shrink-0 text-[var(--primary-color)]"/> <EditableField as="p" path="contactInfo.phone" value={data.contactInfo.phone} onChange={onDataChange} /></div>
                <div className={contactItemClass}><ContactIcon type="linkedin" className="h-4 w-4 shrink-0 text-[var(--primary-color)]"/> <EditableField as="a" href={data.contactInfo.linkedin} target="_blank" rel="noopener noreferrer" path="contactInfo.linkedin" value={data.contactInfo.linkedin} onChange={onDataChange} className={contactLinkClass} validation="url"/></div>
            </div>
        </header>
