...
const MinimalistTemplate: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
    ...
    experience: (
      <section key="experience" className="mb-5">
        ...
        <EditableList items={data.experience || []} path="experience" onChange={onDataChange} newItem={newExperienceEntry}>
          {(exp, index) => (
            <div key={exp.id || index} className="mb-4 break-inside-avoid relative group">
              <div className="flex justify-between items-baseline">
                <h4 style={{ fontSize: `${customization.itemTitleSize}pt` }} className="font-semibold text-black">
                  <EditableField as="span" path={`experience[${index}].role`} value={exp.role} onChange={onDataChange} /> at <EditableField as="span" path={`experience[${index}].company`} value={exp.company} onChange={onDataChange} />
                </h4>
      ...
      <section key="projects" className="mb-5">
        <h3 style={sectionTitleStyles} className="font-semibold mb-3">Projects</h3>
         <EditableList items={data.projects || []} path="projects" onChange={onDataChange} newItem={newProjectEntry}>
          {(proj, index) => (
            <div key={proj.id || index} className="mb-4 break-inside-avoid relative group">
              <h4 style={{ fontSize: `${customization.itemTitleSize}pt` }} className="font-semibold text-black">
    ...
    education: (
      <section key="education" className="mb-5">
        ...
        <EditableList items={data.education || []} path="education" onChange={onDataChange} newItem={newEducationEntry}>
          {(edu, index) => (
            <div key={edu.id || index} className="mb-3 break-inside-avoid relative group">
              <div className="flex justify-between items-baseline">
                <h4 style={{ fontSize: `${customization.itemTitleSize}pt` }} className="font-semibold text-black">
    ...
    skills: (
        <section key="skills" className="mb-5">
          ...
          <EditableList items={data.skills || []} path="skills" onChange={onDataChange} newItem={newSkillCategory}>
              {(category, catIndex) => (
                <p key={category.id || catIndex} className="mb-1 text-black relative group">
  ...
  return (
    <div id="resume-content" style={{ padding: marginValue }}>
      <header className="text-left mb-6 pb-4 border-b-2 border-slate-200">
