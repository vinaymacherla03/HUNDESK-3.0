      ...
      <section key="experience" className="mb-6">
        <h3 style={sectionTitleStyles} className="flex items-center gap-2">
            <span className="w-8 h-8 rounded-full bg-[var(--primary-color)] flex items-center justify-center text-white shadow-sm">
                {/* Replaced problematic SVG icon with a simpler, single-path briefcase icon */}
        ...
        <EditableList items={data.experience || []} path="experience" onChange={onDataChange} newItem={newExperienceEntry}>
          {(exp, index) => (
            <div key={exp.id || index} className="mb-6 pl-4 border-l-2 border-slate-100 ml-4">
              <div className="flex justify-between items-start mb-1">
                ...
                <div className="text-right text-sm text-slate-500 font-mono bg-slate-50 px-2 py-1 rounded">
              ...
              <div className="text-xs text-slate-400 mb-2 flex items-center gap-1">
                  ...
                  {/* FIX: Corrected path for editing location */}
    ...
    projects: (
      <section key="projects" className="mb-6">
        ...
        <EditableList items={data.projects || []} path="projects" onChange={onDataChange} newItem={newProjectEntry}>
          {(proj, index) => (
            <div key={proj.id || index} className="mb-6 pl-4 border-l-2 border-slate-100 ml-4">
              <div className="flex justify-between items-start mb-1">
                <div>
    ...
    education: (
      <section key="education" className="mb-6">
        ...
        <EditableList items={data.education || []} path="education" onChange={onDataChange} newItem={newEducationEntry}>
          {(edu, index) => (
            <div key={edu.id || index} className="mb-6 pl-4 border-l-2 border-slate-100 ml-4">
              <div className="flex justify-between items-start mb-1">
                <div>
              ...
              <div className="text-xs text-slate-400 mb-2 flex items-center gap-1">
               ...
               {edu.relevantCoursework && edu.relevantCoursework.length > 0 && (
                    <p className="text-sm text-slate-600 mt-2"><strong>Relevant Coursework:</strong> <EditableField as="span" path={`education[${index}].relevantCoursework`} value={(edu.relevantCoursework || []).join(', ')} onChange={(p,v)=>onDataChange(p, v.split(',').map(s=>s.trim()))} /></p>
               )}
            </div>
          )}
        </EditableList>
    ...
    certifications: (
      <section key="certifications" className="mb-6">
        ...
        <EditableList items={data.certifications || []} path="certifications" onChange={onDataChange} newItem={newCertificationEntry}>
          {(cert, index) => (
            <div key={cert.id || index} className="mb-6 pl-4 border-l-2 border-slate-100 ml-4">
    ...
    awards: (
      <section key="awards" className="mb-6">
        ...
        <EditableList items={data.awards || []} path="awards" onChange={onDataChange} newItem={newAwardEntry}>
          {(award, index) => (
            <div key={award.id || index} className="mb-6 pl-4 border-l-2 border-slate-100 ml-4">
