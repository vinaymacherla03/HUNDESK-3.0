...
const KyotoTemplate: React.FC<TemplateProps> = ({ data, sectionOrder, customization, sectionVisibility, onDataChange }) => {
  ...
  const Section: React.FC<{title: string; children: React.ReactNode}> = ({ title, children }) => (
    <section className="mb-6 grid grid-cols-4 gap-6">
        <div className="col-span-1">
            <h3 style={{fontSize: `${customization.sectionTitleSize}pt`, color: 'var(--primary-color)'}} className="font-semibold uppercase tracking-wider text-right">
                {title}
            </h3>
        </div>
    ...
    certifications: (
      <Section key="certifications" title="Certifications">
        <EditableList items={data.certifications || []} path="certifications" onChange={onDataChange} newItem={newCertificationEntry}>
          {(cert, index) => (
            <p key={cert.id || index} className="mb-1 text-black">{cert.name}</p>
          )}
        </EditableList>
      </Section>
    ),
    awards: (
      <Section key="awards" title="Awards">
        <EditableList items={data.awards || []} path="awards" onChange={onDataChange} newItem={newAwardEntry}>
          {(award, index) => (
            <p key={award.id || index} className="mb-1 text-black">{award.name}</p>
          )}
        </EditableList>
      </Section>
    ),
  ...
  return (
    <div id="resume-content" style={{ padding: marginValue }} className="relative">
        <div className="absolute left-[29.5%] top-0 bottom-0 w-px bg-slate-200" style={{top: marginValue, bottom: marginValue}} />
